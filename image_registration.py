...
def register_images(base64_image1, base64_image2):
    """
    Registers two images by aligning them based on matched keypoints and descriptors.
    Params:
    ...
    Returns:
        tuple:
            If registration is successful:
                (True, base64 string of first registered image, base64 string of second registered image)
...
def main():
    ...
    if is_registration_successful:
        ...
        plt.imshow(image2)
        plt.subplot(223)
        plt.title("Registered first image")
        plt.imshow(registered_image1)
        plt.subplot(224)
        plt.title("Registered second image")
        plt.imshow(registered_image2)
        plt.tight_layout()
        plt.show()

    else:
        # Get the reason that caused the registration to fail
